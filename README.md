...
## API Key Security
The `.gitignore` file includes `.env`. This means that when committing your code, Git will completely ignore the `.env` file,
 keeping the API keys safe locally. The template file with the variable keys is committed.

Never commit your actual `.env` file with real API keys. Always use the template file for reference.
...
3. **Copy Your Integration Token**
   ...
   - Click "Show" and copy the token (it starts with `ntn_`) as of Jan 22 2026.
...
The application needs a Notion Database that holds the documents describing your company's goals, description, and brand identity.
...
3. **Choose Your Model** (Optional)
   - The default is `nvidia/nemotron-3-nano-30b-a3b:free`
...
Now that you have the necessary API keys, let's set up your environment file:
...
**Verify Your `.env` File is Ignored**
   - Open a terminal in your project folder
   - Run: `git status`
   - You should NOT see `.env` in the list of files
   - If you do see it, that means `.gitignore` isn't working - double check that `.env` is included in the `.gitignore` file.
...
This project can also generate an image via Replicate (model defaults to `sundai-club/pawse`).
...
2. Install deps (if you haven't yet):
...
Run the app and choose **Option 4: Generate image**, then enter an image prompt.
The image will be saved to `outputs/` and the Replicate file URL will be printed (when available).
